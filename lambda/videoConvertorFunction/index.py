...
mediaconvert_client = boto3.client(
    'mediaconvert', endpoint_url=os.environ["MediaconvertEndPoint"])


def lambda_handler(event, context):
    assetID = str(uuid.uuid4())
    sourceS3Bucket = event['Records'][0]['s3']['bucket']['name']
    sourceS3Key = urllib.parse.unquote_plus(
        event['Records'][0]['s3']['object']['key'], encoding='utf-8')
    sourceS3 = 's3://' + sourceS3Bucket + '/' + sourceS3Key
    destinationS3 = 's3://' + os.environ['processingBucket']
    ...
    try:
        ...
        jobSettings['OutputGroups'][0]['OutputGroupSettings']['FileGroupSettings']['Destination'] \
            = destinationS3 + '/' + get_video_convert_key(sourceS3Key)
